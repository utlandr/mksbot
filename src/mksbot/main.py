...
from mksbot.cogs.reddit.reddit import Reddit
...
cogs = [Reddit(bot), Amusement(bot), Music(bot)]
...
async def help(ctx: Context[Bot], *args: str) -> None:
    ...
    if not args:
        ...
        embed.add_field(
            name="Use !help <command> for more information on a specific command",
            value="\u200b",
            inline=False,
        )
        await ctx.send(embed=embed)
...
handler = logging.FileHandler(
    filename="{}/mksbot.log".format(config["bot"]["log_path"]),
    encoding="utf-8",
    mode="w",
)
